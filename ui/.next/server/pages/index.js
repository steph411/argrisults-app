...
const drawChart = (formattedData, chartRef, hour = true) => {
  ...
  let xDateScale = external_d3_["scalePoint"]().domain(labels).range([0, containerWidth - 3]);
  ...
  selection.enter().append("rect").attr("x", (d, i) => i * 31).attr("y", d => containerHeight).attr("width", 30).attr("height", 0).attr("fill", "steelblue").transition().duration(300).attr("height", d => yScale(d)).attr("y", d => containerHeight - yScale(d));
  selection.exit().transition().duration(400).attr("y", d => containerHeight).attr("height", 0).remove();
};
...
})(["margin-top:45px;display:grid;grid-template-columns:339px 1fr;grid-template-rows:200px 330px 100px auto;grid-template-areas:\"location overview\" \"menu forecasts\" \"ads forecasts\" \"ads advisery\";grid-row-gap:40px;column-gap:20px;padding-left:calc(50% - var(--container) / 2);padding-right:calc(50% - var(--container) / 2);.sidebar{display:flex}.location-icon-container{height:202px;width:100%;display:grid;place-items:center;text-align:center;border-radius:10px;background-color:white;box-shadow:var(--shadow-sm);grid-area:location;}.location-icon{width:70px;height:70px;box-shadow:var(--shadow-sm);border-radius:50%;display:grid;place-items:center;background:var(--grey-light);transition:all 300ms ease-out;&:hover{box-shadow:var(--shadow-md);cursor:pointer;}}.location-text{}.overview-container{grid-area:overview;}.overview{width:100%;display:flex;flex-direction:column;height:100%;}.overview-title{font-size:36px;padding:10px 0px;}.overview-content{width:100%;display:flex;margin-top:auto;justify-content:space-between;align-self:end;}"]);
...
const getForecasts = async (lat, lon, accessToken = undefined) => {
  ...
  const req = await external_axios_default.a.get(url, {
    headers: {
      Authorization: `Bearer ${access_token}` // "Access-Control-Allow-Origin": "*",
...
const getDailyWeather = async (lat, lon, accessToken = undefined) => {
  let access_token = accessToken;
...
function Home({
  ...
  const updateLonLat = async (lat, lon) => {
    try {
      ...
      setAppData(old => _objectSpread(_objectSpread({}, old), {}, {
        dailyWeather,
        forecasts
      }));
